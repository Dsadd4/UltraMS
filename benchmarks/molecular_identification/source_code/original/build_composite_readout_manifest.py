#!/usr/bin/env python3
"""Freeze the composite-polarity Figure 3h split for matched readout training.

This reproduces the original ``08_libsearch.py`` data scope: positive and
negative *composite adduct sets*, raw stripped SMILES compound identities,
compounds with at least two valid spectra, a 70/15/15 compound split with seed
42, and one frozen query per validation/test compound with seed 119.
"""
...
RARE_POSITIVE_ADDUCTS = {"[M]+", "[M+H-2H2O]+", "[M-H2O]+"}
RARE_NEGATIVE_ADDUCTS = {"[M]-"}
...
def build_library(
    ...
    rows, smiles = [], []
    # The composite producer iterates compounds in sorted raw-SMILES order.
...
def build(args: argparse.Namespace) -> None:
    ...
    mode_sets = {
        "pos": POSITIVE_ADDUCTS | (RARE_POSITIVE_ADDUCTS if args.include_rare_adducts else set()),
        "neg": NEGATIVE_ADDUCTS | (RARE_NEGATIVE_ADDUCTS if args.include_rare_adducts else set()),
    }
    with h5py.File(h5_path, "w") as h5:
        ...
        for source_row, row in enumerate(read_csv_rows(args.csv)):
            ...
            mode = "pos" if adduct in mode_sets["pos"] else "neg" if adduct in mode_sets["neg"] else None
            if mode is None:
                continue
            raw_counts[mode] += 1
            spectrum = producer_preprocess(row["mzs"], row["intensities"], max_peaks=150)
            raw_smiles = str(row.get("smiles", "")).strip()
            # Keep the exact compound identity used by the frozen producer.
            # Canonicalising here would change split membership and invalidate
            # the cached UltraMS/DreaMS query and library identities.
            smiles = "" if raw_smiles in ("", "nan") else raw_smiles
        ...
        summary: dict[str, object] = {
            ...
            "include_rare_adducts": bool(args.include_rare_adducts),
            "mode_labels": {"pos": "positive composite adducts", "neg": "negative composite adducts"},
            "compound_identity": "raw stripped MSnLib SMILES, matching frozen 08_libsearch.py",
            "adduct_sets": {mode: sorted(values) for mode, values in mode_sets.items()},
            "modes": {},
        }
        for mode, writer in writers.items():
            # The composite producer removes compounds with fewer than two
            # spectra before splitting.
            singleton_spectra = sum(len(rows) for rows in writer.row_ids_by_smiles.values() if len(rows) == 1)
            valid_rows = {
                smiles: rows for smiles, rows in writer.row_ids_by_smiles.items()
                if len(rows) >= 2
            }
            mode_split_seed = args.split_seed
            ...
            mode_summary = {
                ...
                "singleton_spectra_excluded_from_protocol": singleton_spectra,
...
def main() -> None:
    ...
    parser.add_argument("--reference-embedding-dir", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--split-seed", type=int, default=42)
    parser.add_argument("--query-seed", type=int, default=119)
    parser.add_argument("--include-rare-adducts", action="store_true")
