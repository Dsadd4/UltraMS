...
def get_collision_energy(filename):
    colli_eng = re.findall(r'collision +([0-9]+\.?[0-9]*|nan).*', filename)
    if len(colli_eng) > 1:
...
def has_valid_elements(smiles: str) -> bool:
    """
    Check if a SMILES string contains only valid elements supported by the model.

    Args:
        smiles (str): SMILES string to check

    Returns:
        bool: True if all elements are supported, False otherwise
    """
    try:
        mol = Chem.MolFromSmiles(smiles)
        if mol is None:
        ...
        # Check if all elements are in the valid set
        invalid_elements = atom_symbols - set(VALID_ELEMENTS)
        return len(invalid_elements) == 0
    except Exception:
        return False

from typing import Dict
def filter_batch_by_valid_elements(batch_data: List[Dict]) -> tuple:
    """
    Filter a batch of molecules to keep only those with valid elements.

    Args:
        batch_data (List[Dict]): List of dictionaries containing 'smiles' key

    Returns:
        tuple: (filtered_batch, filtered_indices, skipped_count)
               - filtered_batch: List of valid molecules
               - filtered_indices: Original indices of kept molecules
               - skipped_count: Number of molecules skipped due to invalid elements
    """
    filtered_batch = []
    filtered_indices = []
    skipped_count = 0
    ...
    for idx, item in enumerate(batch_data):
        if has_valid_elements(item.get('smiles', '')):
            filtered_batch.append(item)
            filtered_indices.append(idx)
        else:
            skipped_count += 1

    return filtered_batch, filtered_indices, skipped_count
