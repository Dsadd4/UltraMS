"""Extract the frozen UltraMSdata training corpus without changing rows.
...
from .prepare_ultramsdata import UltraMSdataConfig, prepare_ultramsdata
...
def verify_source(src: Path, contract: dict, expected_fingerprint: str) -> dict:
    ...
    require(first_name.endswith("shard_0000.parquet"), "unexpected source shard sequence")
    shard_prefix = first_name[: -len("shard_0000.parquet")]
    expected_ultramsdata = [
        f"{shard_prefix}shard_{i:04d}.parquet"
    ...
    require(
        source_names
        == expected_ultramsdata + [f"public_{i:04d}.parquet" for i in range(len(public))],
        "input provenance inventory differs",
    )
    for item, (_, rows, digest) in zip(inputs["files"][len(expected_ultramsdata) :], public):
        require(
    ...
    pure_rows = h["clean_ultramsdata_rows"]
    require(
    ...
    for index in (boundary - 1, boundary):
        ...
        if index < boundary:
            require(
                cursor["source_index"] == len(expected_ultramsdata) - 1,
                "pre-boundary cursor is not in last UltraMSdata source",
            )
        else:
            require(
                cursor["source_index"] >= len(expected_ultramsdata),
...
def extract_ultramsdata(
    ...
    *,
    ...
    for directory in (work_dir, dst):
        owner = directory / "ultramsdata_owner.json"
    ...
    if manifest.get("status") == "complete":
        ...
        require(
            ...
            == contract["historical_replay"]["clean_ultramsdata_polarity"],
        ...
        atomic_write_json(
            dst / "ultramsdata_provenance.json",
            {
                "status": "passed",
                **ownership,
...
def main() -> None:
    ...
    parser.add_argument("--dataset-name", default="UltraMSdata")
    parser.add_argument("--output-shards", type=int, default=816)
    parser.add_argument("--world-size", type=int, default=4)
    args = parser.parse_args()
    result = extract_ultramsdata(
        args.src,
        args.dst,
        args.work_dir,
        args.contract,
