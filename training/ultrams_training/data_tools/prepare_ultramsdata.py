#!/usr/bin/env python3
"""Prepare the clean UltraMSdata training shards.

Inputs are read in lexical basename order as one global row stream. Configured
partition and precursor-mass selection preserves the order of retained rows.
The extraction entry point disables selection when it repartitions the frozen
UltraMSdata corpus.
...
def _validate_config(config: UltraMSdataConfig) -> None:
...
def resolve_output_shard_rows(config: UltraMSdataConfig) -> list[int] | None:
