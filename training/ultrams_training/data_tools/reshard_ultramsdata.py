#!/usr/bin/env python3
"""Reshard UltraMSdata Parquet files while preserving spectrum rows.
...
class UltraMSdataReshardConfig:
    source_dir: Path
    output_dir: Path
...
def _validate_config(config: UltraMSdataReshardConfig) -> None:
...
def _build_or_validate_input_manifest(
    config: UltraMSdataReshardConfig, audit_dir: Path
) -> dict[str, Any]:
...
def _write_and_audit_temporary(
    table: pa.Table,
    temporary: Path,
    config: UltraMSdataReshardConfig,
...
def reshard_ultramsdata(
    ...
    *,
    ...
    """Build or resume a deterministic UltraMSdata reshuffle."""
